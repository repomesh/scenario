...
You may still worry about non-determinism, but remember, each scenario is designed to test one thing, very specifically. Once it works (especially with lower temperature settings), it will stay stable. Your prompts and agent architecture will keep evolving, but your tests, once solid, provide a dependable safety net.
