...
Configurable via `success_score` and `success_confirm_turns` on `RedTeamAgent.crescendo()`.

<Callout type="info">
  Python only for now. TypeScript support is coming — see the roadmap below.
</Callout>

### 5. Backtracking on stalls

When the scorer returns a score at or below `backtrack_threshold` (default 1) for `success_confirm_turns` consecutive turns, the attacker backtracks: it removes the last N user+assistant pairs from the conversation and injects a meta-instruction telling the attacker model to try a completely different approach.

This prevents the attacker from getting stuck in a loop of identical refusals. After exhausting `max_backtracks` (default 3) backtrack attempts, the attacker continues linearly for the remaining turns.

Configurable via `max_backtracks` and `backtrack_threshold` on `RedTeamAgent.crescendo()`.

<Callout type="info">
  Python only (`feat/red-team-early-exit` branch). TypeScript support is coming.
</Callout>
...
scenario.RedTeamAgent.crescendo(
    ...
    # Early exit
    success_score=9,                        # score threshold for early exit
    success_confirm_turns=2,                # consecutive turns at threshold to confirm

    # Backtracking
    max_backtracks=3,                       # max backtrack attempts before continuing linearly
    backtrack_threshold=1,                  # score at or below this triggers backtrack
...
scenario.redTeamCrescendo({
  ...
  // Early exit — not yet available in TypeScript
  // successScore: 9,
  // successConfirmTurns: 2,

  // Backtracking — not yet available in TypeScript
  // maxBacktracks: 3,
  // backtrackThreshold: 1,
...
| **Early exit** | | | | |
| Success score | `success_score` | — | `9` | Score threshold (0–10) that counts as success. |
| Confirm turns | `success_confirm_turns` | — | `2` | Consecutive turns at `success_score` before ending early. |
| **Backtracking** | | | | |
| Max backtracks | `max_backtracks` | — | `3` | How many times the attacker can backtrack before continuing linearly. |
| Backtrack threshold | `backtrack_threshold` | — | `1` | Score at or below this for consecutive turns triggers a backtrack. |
| **Temperature** | | | | |
    ...
    result = await scenario.run(
        ...
        agents=[
            ...
            scenario.RedTeamAgent.crescendo(
                ...
                detect_refusals=False,      # no refusal detection either
            ),
            scenario.JudgeAgent(criteria=[...]),
        ],
...
With backtracking enabled (Python), even 10-turn runs are effective — the attacker recovers from dead-end approaches instead of wasting turns repeating the same failed technique.
...
- **Backtracking for TypeScript** ([#2043](https://github.com/langwatch/scenario/issues/2043))
