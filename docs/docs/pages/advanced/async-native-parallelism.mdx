---
title: Async-Native Parallelism - scenario.arun
description: Run scenarios on the caller's event loop with `scenario.arun`. The default `scenario.run` already parallelises via a thread pool, so reach for `arun` only when your codebase is fully async-first.
---

import { Callout } from "vocs/components";

# Async-Native Parallelism [When to prefer scenario.arun over the default threaded run]
